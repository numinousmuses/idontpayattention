...
import { FileText, HardDrive, Share2, Download } from "lucide-react";
...
export default function ForeverFreeSection() {
  ...
  return (
    <section className="w-full py-16 bg-blue-50 relative">
      {/* Grid background */}
      <div 
      ...
      <div className="relative flex w-full overflow-x-hidden z-10">
          <div className="animate-marquee whitespace-nowrap py-8 flex items-center">
            {extendedFeatures.map((feature, index) => {
              ...
              return (
                <Card key={index} className={`w-96 h-56 p-6 ${bgColor} border-2 border-black shadow-lg flex-shrink-0 flex flex-col justify-center mx-4`}>
                  <div className="mb-3">
          ...
          <div className="absolute top-0 animate-marquee2 whitespace-nowrap py-8 flex items-center">
            {extendedFeatures.map((feature, index) => {
              ...
              const bgColor = index % 2 === 0 ? "bg-blue-400" : "bg-blue-300";
              return (
                <Card key={`duplicate-${index}`} className={`w-96 h-56 p-6 ${bgColor} border-2 border-black shadow-lg flex-shrink-0 flex flex-col justify-center mx-4`}>
                  <div className="mb-3">
                    <Icon size={36} className="text-black" />
                  </div>
                  <h3 className="text-lg font-bold text-black mb-2 line-clamp-2">
